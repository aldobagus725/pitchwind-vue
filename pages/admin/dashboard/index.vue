  ...
  <script>
    export default {
      ...
      async asyncData({ $axios }) {
        ...
        //fetching dashboard
        const dashboard = await $axios.$get('/api/v1/admin/dashboard')
        console.log(dashboard)
        //statistic
        const statistic = {
          'pending': dashboard.data.data.count.pending,
          'success': dashboard.data.data.count.success,
          'expired': dashboard.data.data.count.expired,
          'failed': dashboard.data.data.count.failed,
        }
  
        //cart
        const chart = {
          chartData: {
            labels: dashboard.data.data.chart.month_name,
            datasets: [
              {
                label: `STATISTIK PENDAPATAN : ${new Date().getFullYear()}`,
                backgroundColor: '#bccad8',
                data: dashboard.data.data.chart.grand_total
              },
            ]
          }
        }
