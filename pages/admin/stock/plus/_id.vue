<template>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="number" v-model="product.add_stock_qty" placeholder="Masukkan Penambahan"
                                    class="form-control">

                                <br />
                                <h4>Stock Final : {{parseInt(product.stock) + parseInt(product.add_stock_qty)}}</h4>
                                <div v-if="validation.stock" class="mt-2">
                                    <b-alert show variant="danger">{{ validation.add_stock_qty[0] }}</b-alert>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
  ...
  <script>
    export default {
      ...
      head() {
        return {
          title: 'Edit Stock (Plus) - Administrator',
        }
      },
      ...
      methods: {
        ...
        async updateStock() {
          ...
          //define formData
          let data = {
            add_stock_qty : this.product.add_stock_qty,
            stock : this.product.stock,
            description : this.product.description,
            stock_final :  parseInt(this.product.stock) + parseInt(this.product.add_stock_qty)
          }
          //sending data to action "updateProduct" vuex
          await this.$store.dispatch('admin/stock/storePlusStock', {
